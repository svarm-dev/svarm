...
- **Optional review proof-of-work checklist** ([#239](https://github.com/svarm-dev/svarm/issues/239)): WORKFLOW front matter `review.checklist` (string ids or `{id, label}`) turns the review Evidence pack into a checklist with `pass` / `fail` / `pending` / `unknown` / `na` from existing signals only (`pr` from the PR URL, `ci` from the CI chip, `cost` from the cost receipt). Custom ids keep their label and evaluate `unknown`; malformed entries are skipped. Non-empty checklists append item labels to the agent prompt (informational). Never gates merge, dispatch, or mark-done.
...
- **Board Abort** ([#186](https://github.com/svarm-dev/svarm/issues/186)): run console **Abort** stops a live CLI or PiRPC run with the same OS kill-tree as stall/timeout (not only `Process.exit/2`). Same `board_auth_at` gate as approve / steer / answer. Transcript shows muted `[board] aborted`. Ticket returns to **`todo`** (not crash-retry of the same run; gated assignees re-enter the approval gate; trusted assignees may be picked up on the next poll). Tracker PATCH is fail-closed: if `update_status` errors, the board does not claim Todo (the OS tree may already be dead).
