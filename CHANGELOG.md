...
- **GitHub `get_issue` for non-eligible statuses** ([#174](https://github.com/svarm-dev/svarm/issues/174)): lookup by GraphQL `node_id` scans `list_issues` (`state: all`, board-visible) instead of `list_eligible` (open + dispatchable). Approve / complete-review / retry re-fetch of a GitHub card still resolve after the issue leaves the eligible set. Numeric `source_id` still uses REST `GET /issues/{number}`. Residual: both paths are bound to the configured owner/repo and the list scan is a single page — transferring an issue to another repository is not followed (no GraphQL `node(id:)`).
