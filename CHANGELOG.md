...
- **GitHub `pending_approval` labels** ([#173](https://github.com/svarm-dev/svarm/issues/173)): default maps persist `pending_approval` as `status: pending-approval`; budget hold reuses that status plus `wait_reason` (no extra label). WORKFLOW `tracker.status_labels` / `reverse_labels` are parsed; invalid maps fail closed.
