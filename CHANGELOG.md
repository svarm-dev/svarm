...
- **Steer live PiRPC runs** ([#150](https://github.com/svarm-dev/svarm/issues/150), epic [#53](https://github.com/svarm-dev/svarm/issues/53)): run console **Steer** queues pi RPC `type: steer` on a live session (same `board_auth_at` as approve/answer). CLI disabled; hidden while a mid-run question is parked. Follow-up-after-settle is not in this slice.
