...
| Pull requests | Read & write | Open PR; poll submitted reviews; review-resume spawn writes the same issue status as CI resume |
| **Checks** | **Read** | CI resume: poll check-runs on managed PRs (optional feature) |

Webhook: disabled for v1 (poll loop). CI resume and review-resume also use poll-on-tick, not webhooks.
