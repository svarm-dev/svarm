...
- **Review Station** — structured Evidence (PR, attempts, agent/model, cost, age) on selected review cards; PR/no-PR glance chips and a CI `pass` / `fail` / `pending` / `unknown` summary chip (N/A on the local tracker). Optional WORKFLOW `review.checklist` (string ids or `{id, label}`) turns Evidence into a proof-of-work checklist with `pass` / `fail` / `pending` / `unknown` / `na`, echoed to the agent prompt as labels. Informational only — never gates merge or dispatch; humans still merge on GitHub.
